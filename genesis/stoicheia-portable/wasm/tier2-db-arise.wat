;; Tier 2 DB Arise - WASM module for entity creation
;; V9.5 portable stoicheion
;;
;; Delegates to host via dynamis.db_arise import

(module
  ;; Import the host function from dynamis namespace
  ;; db_arise(eidos_ptr, eidos_len, id_ptr, id_len, data_ptr, data_len, cf_ptr, cf_len)
  ...
  ;; Arise (create) an entity - delegates to host via dynamis import
  (func (export "arise")
    (param $eidos_ptr i32) (param $eidos_len i32)
    (param $id_ptr i32) (param $id_len i32)
    (param $data_ptr i32) (param $data_len i32)
    (param $cf_ptr i32) (param $cf_len i32)
    (result i32 i32)
    (call $db_arise
      (local.get $eidos_ptr) (local.get $eidos_len)
      (local.get $id_ptr) (local.get $id_len)
      (local.get $data_ptr) (local.get $data_len)
      (local.get $cf_ptr) (local.get $cf_len))
  )
)
