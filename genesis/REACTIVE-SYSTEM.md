...
3. **Substrate abstraction** for external resources (actuality modes)
...
| Actuality Mode | "How to touch substrate?" | Reconciler action | dynamis |
...
- eidos: reconciler
  ...
  data:
    ...
    # How to find actuality mode
    actuality:
      mode_field: actuality_mode      # or literal: "process"
...
## Layer 3: Actuality Mode System

### Purpose

An **actuality mode** is a category of substrate manifestation. It answers: "In what manner does this type of entity become actual?"
...
Every actuality mode defines three operations:
...
The interpreter looks up actuality modes at runtime:

```
Entity declares: actuality_mode = "process", provider = "local"
    ↓
Lookup: actuality-mode/process-local
    ↓
...
- [dynamis/DESIGN.md](dynamis/DESIGN.md) — Full actuality mode specification
- [dynamis/actuality-modes/dynamis.yaml](dynamis/actuality-modes/dynamis.yaml) — Existing mode definitions
...
A reflex can trigger a reconciler, but the reconciler's Tier 3 operations still require the invoking animus to hold the appropriate attainment.
...
pub fn manifest(&self, entity_id: &str) -> Result<Value> {
    ...
    let mode = entity.get("data.actuality_mode").as_str()
        .unwrap_or("process");
    let provider = entity.get("data.provider").as_str()
        .unwrap_or("local");

    // 3. Lookup actuality mode (generated dispatch table)
    let stoicheion_name = actuality_stoicheion(mode, provider, ActualityOperation::Manifest)
        .ok_or_else(|| KosmosError::NotFound(
            format!("No manifest stoicheion for {}/{}", mode, provider)
        ))?;

    // 4. Map params from entity data
    let actuality_mode = self.find_entity(
        &format!("actuality-mode/{}-{}", mode, provider)
    )?;
    let param_names = actuality_mode
...
### Reconciler Loop (reconciler_loop.rs)

```rust
pub struct ReconciliationLoop {
    interval: Duration,
    reconcilers: Vec<String>,  // reconciler IDs to run
}

impl ReconciliationLoop {
    pub async fn run(&self, host: Arc<HostContext>) {
        loop {
            for reconciler_id in &self.reconcilers {
                // Gather all entities for this reconciler
                let reconciler = host.find_entity(reconciler_id)?;
                let target_eidos = reconciler.get("data.target_eidos").as_str()?;
                let entities = host.gather_entities(Some(target_eidos), None, None)?;

                // Reconcile each
                for entity in entities {
                    if let Err(e) = host.invoke_praxis(
                        "dynamis/reconcile",
                        json!({
                        ...
                        eprintln!("[reconcile] {} failed: {}", entity.get("id"), e);
                    }
                }
            }
  ...
  data:
    ...
    fields:
      ...
      actuality_mode:
        ...
        required: true
        default: process
        description: "Actuality mode (process, object-storage, dns)"
...
| Oikos | Reflex | Trigger | Response |
|-------|--------|---------|----------|
| demiurge | artifact-added | bond_created (contains → oikos) | update-manifest |
...
| ergon | pragma-signaled | bond_created (signals-to → circle) | add-notification |
| ergon | pragma-resolved | entity_updated (pragma) | add-notification |
| nous | theoria-created | entity_created (theoria) | index-entity |
| nous | expression-created | entity_created (expression) | index-entity |
...
| Praxis | Oikos | Purpose |
|--------|-------|---------|
| demiurge/update-manifest | demiurge | Update oikos manifest on artifact add |
...
| ReconcilerLoop | reconciler_loop.rs | Parasympathetic drift detection (30s interval) |
...
|-----------|-------------|
| **Schema-driven** | Reflexes, reconcilers, and actuality modes are all YAML definitions interpreted at runtime. Adding new behaviors = editing YAML, not code. |
...
| **Composition-only** | Reflexes compose trigger + response. Reconcilers compose intent + actuality + transitions. Actuality modes compose operations + stoicheia. |
...
| Path | Contents |
|------|----------|
| [demiurge/entities/reflexes.yaml](demiurge/entities/reflexes.yaml) | 3 oikos development reflexes |
...
- [dynamis/DESIGN.md](dynamis/DESIGN.md) — Reconciler and actuality mode specification
