...
Added to **dynamis** oikos (desmoi connecting deployment to soma infrastructure):
...
| **steward-of** | circle → node | Circle governs commons node |
...
- eidos: desmos
  ...
  data:
    name: steward-of
    description: |
      Circle stewards (governs) this commons node.
      Replaces the embedded node.steward_circle_id field with a proper bond.
      The steward circle has authority over node configuration and service allocation.
    from_eidos: circle
    to_eidos: node
    cardinality: one-to-many
    symmetric: false
    note: |
      A commons node has exactly one steward circle.
      Query: from node, trace steward-of (reverse) to find governing circle.
```

**Use cases:**
- `circle/liminal-commons` steward-of `node/chora-node`
...
The soma oikos already provides:
...
- `provides-to` desmos — service provides to circle
...
3. **Stewardship Validation**
   - Before manifesting on a commons node, verify circle has stewardship
...
Scenario: Deploy kosmos-server serving vibe-cafe circle on chora-node.
...
bind(circle/liminal-commons, node/chora-node, steward-of)
...
- Can trace `steward-of` from circle to node? → Can deploy
...
- `authorized-by` → circle with deploy attainment
