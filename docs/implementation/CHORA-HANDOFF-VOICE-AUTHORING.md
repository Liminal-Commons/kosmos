...
> **Status:** Active
> **Waiting on:** Voice UI and authoring-session rendering in chora

This document describes the voice-authoring topos implementation in kosmos and what needs to be built in chora to actualize it.

---

## Overview

**Voice-authoring** is a mode (topos) where discussion drives authorship. Humans speak about a focused artifact, and the artifact updates through conversation. No hand-editing — the creative act is conversational.

**The insight**: The artifact at any moment is self-sufficient. It doesn't need all prior phaseis to be meaningful. The conversation shapes it; the result stands alone.
...
├── manifest.yaml                # Topos declaration
...
| `informed-by` | artifact | theoria, artifact, phasis | What shaped generation |
| `discusses-about` | phasis | artifact | Optional provenance |
...
| `process-authoring-phasis` | **Core operation** — phasis → artifact update |
| `create-artifact-from-conversation` | Synthesize thread into new artifact |

### Attainment

`attainment/voice-author` is defined in voice-authoring eide and granted by `oikos/kosmos` in spora.yaml.

Requires:
- `attainment/perceive` (voice capture)
- `attainment/express` (commit phaseis)
- `attainment/render` (visual display)
- `attainment/manteia` (governed inference)

### Inference Context (new typos)

Created in `genesis/manteia/typos/manteia.yaml`:
...
│     artifact-preview        │     phasis-thread       │
│                             │                             │
├─────────────────────────────┴─────────────────────────────┤
...
2. Wire up panel components:
   ...
   - `PhasisThread` — existing, filter to session phaseis
...
### 3. Phasis Commit Hook

When a phasis is committed in voice-authoring mode:

```typescript
// In phasis commit handler
if (isInVoiceAuthoringMode()) {
  const session = getActiveAuthoringSession();
  await callPraxis('voice-authoring/process-authoring-phasis', {
    session_id: session.id,
    phasis_id: newExpression.id
  });
}
```
...
5. Update artifact
6. Bond provenance
7. Create kosmos phasis response
...
**UI consideration:**
- Style kosmos phaseis differently (different avatar, color)
...
| `phasis` (in oikos) | New human/kosmos phaseis |
...
phasis (thyra)
    ...
    ▼
process-authoring-phasis (voice-authoring)
    │
...
update artifact entity
    │
    ├─► bind informed-by → phasis
    ├─► bind informed-by → theoria[]
    ├─► bind discusses-about (phasis → artifact)
    │
    ▼
compose kosmos phasis response
    │
    ▼
...
4. Phasis commit hook wiring

### Phase 2: Update Flow
1. `process-authoring-phasis` implementation
...
# Test phasis processing
voice-authoring_process-authoring-phasis session_id="..." phasis_id="..."
```
...
| `genesis/voice-authoring/manifest.yaml` | Topos declaration |
...
| `genesis/manteia/typos/manteia.yaml` | Inference context typos |
...
| `voiceAuthoringCommitHook.ts` | Phasis commit integration |
...
5. **Mode is a way of dwelling** — Not a feature, a topos. HUD shifts. Capabilities shift.
