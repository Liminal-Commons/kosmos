...
- **Kosmos repo**: Where dwelling happens (composition, crystallization, topos development)
...
This proposal introduces **pragma** (πρᾶγμα — "thing to be done") as the coordination primitive between oikoi.
...
eidos/pragma:
  ...
  description: |
    ...
    - Choran oikos accepts and resolves (capability)
...
praxis/ergon/list-pragma:
  ...
  steps:
    - step: set
      bindings:
        target_circle: "$oikos_id"

    - step: switch
      cases:
        - when: "not $target_oikos"
          then:
            - step: set
              bindings:
                target_circle: "$_oikos"
...
3. Create pragma:
   ...
   ergon/create-pragma
     ...
     target_oikos_id: "oikos/chora-dev"
...
- Or: pragma are exported/imported between oikos instances
...
- **ergon** (work) — new topos focused on work items and coordination
- **politeia** — extends governance with work signaling
- **hypostasis** — extends federation with pragma exchange

Recommend **ergon** as a focused topos for work coordination.
...
|----------|---------|-------------------|
| `invitation` | Membership request (join oikos) | Pragma is work request (do thing) |
...
| `syndesmos` | Oikos federation link | Pragma flows over syndesmos |
...
**T27: Same parousia, multiple thyrai**

Claude Code is thyra, not parousia. One Victor, one parousia, multiple interfaces. The pragma flow works because it's the same dweller moving between contexts, not separate agents trying to coordinate. The discipline boundary is between oikoi (kosmos vs chora), not between parousiai.
...
2. Create ergon topos with manifest
...
*Drafted 2026-01-27 — from conversation about cross-oikos coordination and discipline boundaries*
